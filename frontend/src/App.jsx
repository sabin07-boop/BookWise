  ...
  if (loading) {
    return (
      ...
      >
        <div className="discovery-section-heading">
          <div>
            <span className="discovery-eyebrow">
              {sectionKind === "suggested" ? "FOR YOU" : "TRENDING NOW"}
            </span>
            <h2>{title}</h2>
        ...
        <div className="book-carousel discovery-loading-row" aria-hidden="true">
          {[1, 2, 3, 4, 5].map((item) => (
            <div className="discovery-skeleton-card" key={item}>
              <div className="discovery-skeleton-cover" />
              <div className="discovery-skeleton-line wide" />
  ...
  return (
    ...
    >
      <div className="discovery-section-heading">
        ...
        <div className="discovery-heading-meta">
          <span className="discovery-count">{visibleBooks.length} picks</span>
          <span className="discovery-scroll-hint">Swipe to explore →</span>
        </div>
      </div>

      <div className="book-carousel-wrapper discovery-carousel">
        {canScrollLeft && (
          <button
            type="button"
...
function BookDetailModal({
  ...
  useUsageTracker({
    token,
    enabled: Boolean(token && bookIdentifier && !commentsSectionVisible),
    activityType: "book",
    resourceKey: String(bookIdentifier || ""),
    resourceName: book?.title || "Book",
  });
  ...
  useUsageTracker({
    ...
    resourceKey: String(bookIdentifier || ""),
    resourceName: book?.title ? `${book.title} · Comments` : "Comments",
  });
            ...
            items.map((item) => (
              ...
              >
                <img
                  src={
                    item.cover_url ||
                    "https://via.placeholder.com/60x85?text=Book"
                  }
                  alt={item.title}
                  style={{
                    width: 60,
    ...
    >
      ...
      >
        ...
        >
          ...
          <strong style={{ display: "block", fontSize: 25, marginTop: 3 }}>
        ...
        {!method && (
          <div style={{ display: "grid", gap: 10 }}>
            ...
            >
              ...
              <small style={{ color: "#64748b" }}>
                Transfer to our bank account and submit the reference
              </small>
            </button>
            ...
            >
              ...
              <small style={{ color: "#64748b" }}>
                Pay using the Bitcoin address and QR
              </small>
            </button>
            ...
            >
              ...
              <small style={{ color: "#64748b" }}>
                USDT on TON — address and QR below
              </small>
            </button>
          </div>
        )}
        ...
        {method && (
          <div>
            ...
            {method === "bank" && (
              ...
              >
                ...
                <p style={{ color: "#475569", fontSize: 13 }}>
                  Transfer exactly the order amount, then enter your bank
                  transaction/reference number below. Your order stays pending
                  until an admin verifies the transfer.
                </p>
                <div style={{ textAlign: "center", margin: "12px 0 16px" }}>
                  <img
                    src={BANK_QR_SRC}
                    alt="BookWise bank transfer details QR code"
                    style={{
                      width: 210,
                      height: 210,
                      background: "#fff",
                  ...
                  <div style={{ fontSize: 12, color: "#64748b", marginTop: 8 }}>
                    Scan to view the BookWise bank transfer details
                  </div>
                </div>
                <div style={{ display: "grid", gap: 8, fontSize: 14 }}>
                  <div>
                    <strong>Bank:</strong>{" "}
                    {options?.bank?.name || "Not configured"}
                  </div>
                  <div>
                    <strong>Account name:</strong>{" "}
                    {options?.bank?.accountName || "Not configured"}
                  </div>
                  <div>
                    <strong>Account number:</strong>{" "}
                    {options?.bank?.accountNumber || "Not configured"}
                  </div>
                  <div>
                    <strong>Branch:</strong>{" "}
                    {options?.bank?.branch || "Not configured"}
                  </div>
                  {options?.bank?.swift && (
                    <div>
                      <strong>SWIFT:</strong> {options.bank.swift}
                    </div>
                  )}
                </div>
                <input
            ...
            {selectedCrypto && (
              ...
              >
                <h3 style={{ marginTop: 0 }}>
                  {method === "btc" ? "Bitcoin payment" : "USDT payment"}
                </h3>
                <img
                  src={method === "btc" ? BTC_QR_SRC : USDT_QR_SRC}
                  alt={`${method === "btc" ? "Bitcoin" : "USDT"} payment QR`}
                  style={{
                    width: 210,
                    height: 210,
                    background: "#fff",
                ...
                >
                  Scan this QR or copy the address. Send the equivalent of{" "}
                  <strong>NPR {Number(total).toLocaleString()}</strong>. Crypto
                  payments are manually verified before the order is marked
                  paid.
                </p>
                {method === "usdt" && (
                  <p
                    style={{ fontSize: 12, color: "#b45309", fontWeight: 800 }}
                  >
                    Network: TON. Send USDT only over TON to this address.
                  </p>
                )}
                <div
                  style={{
                ...
                >
                  <input
                    readOnly
                    value={method === "btc" ? BTC_ADDRESS : USDT_ADDRESS}
                    style={{
                      flex: 1,
                      minWidth: 0,
                      padding: 10,
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigator.clipboard?.writeText(
                        method === "btc" ? BTC_ADDRESS : USDT_ADDRESS,
                      )
                    }
                    style={{
                      border: 0,
            ...
            <button
              type="button"
              disabled={loading || (method === "bank" && !reference.trim())}
        ...
        >
          eSewa is verified automatically by the server. Bank and crypto
          payments are recorded as pending and require manual verification
          before fulfillment.
        </p>
      </div>
    </div>
...
function AuthModal({ mode, setMode, onClose, onSuccess }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (isRegisterVerify) {
        ...
        setMessage(
          data.message || "Email verified and account created successfully.",
        );
        onSuccess(data);
        return;
      }
  ...
  const resendCode = async () => {
    ...
    try {
      ...
      const body = isRegisterVerify ? { name, email, password } : { email };
      ...
      setMessage(
        data.message || "A new 5-digit verification code has been sent.",
      );
      setCode("");
    } catch (error) {
    ...
    >
      <div className="auth-modal">
        ...
        {(isRegisterVerify || isResetVerify) && (
          ...
          >
            Check your inbox for the <strong>5-digit code</strong>. It expires
            in 10 minutes.
          </div>
        )}
          ...
          {(currentMode === "login" ||
            ...
            <div className="form-group">
              ...
              <input
                ...
                required
                disabled={isResetVerify || isRegisterVerify}
              />
            </div>
          )}
        ...
        <div className="auth-switch">
          {currentMode === "login" ? (
            ...
            <button type="button" onClick={goBackToRegister}>
...
function AdminPortal({ onBackToBookWise, onAdminSuccess }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          data.error || data.message || "Admin authentication failed.",
        );
      if (data.token) localStorage.setItem("token", data.token);
      if (data.user) localStorage.setItem("user", JSON.stringify(data.user));
      onAdminSuccess(data);
      ...
      <div className="admin-auth-card">
        ...
        <p>
          {mode === "login"
            ...
            : "A current administrator must generate a 6-digit invitation code before registration."}
        </p>
        ...
        <form onSubmit={submit} className="admin-auth-form">
          ...
          {mode === "register" && (
            <label>
              ...
              />
              <small>
                Get this code from an existing BookWise administrator.
              </small>
            </label>
          )}
          <button
...
function SupportWidget({ user, books = [] }) {
  // The customer always starts with the AI assistant. An agent conversation
  // is created only after the customer explicitly chooses "Transfer to agent".
  ...
  const suggestedBooks = useMemo(() => {
    ...
    return [...list]
      ...
      .sort((a, b) => {
        ...
        const yearA = Number(a.published_year || 0);
  ...
  const loadConversation = async (id = conversationId) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(
        `${API_URL}/api/support/conversations/${id}`,
        { headers: authHeaders() },
      );
      const data = await response.json().catch(() => ({}));
      ...
      if (!response.ok) {
        throw new Error(
          data.error ||
            data.detail ||
            `Unable to load support chat (${response.status}).`,
        );
      }
      ...
      if (
        /access to this conversation|conversation not found|unable to load support chat/i.test(
          String(err.message || ""),
        )
      ) {
  ...
  useEffect(() => {
    if (!open || !conversationId || mode !== "agent") return;
  ...
  const createAgentConversation = async (
    ...
    const response = await fetch(`${API_URL}/api/support/conversations`, {
      ...
      body: JSON.stringify({
        visitorToken,
        message: `🔔 Agent transfer requested. ${handoffReason}`,
        mode: "agent",
        source: "ai_chatbot",
      }),
    });
    ...
    if (!response.ok) {
      throw new Error(
        data.error ||
          data.detail ||
          `Agent transfer failed (${response.status}).`,
      );
    }
  ...
  const getLocalAiReply = (text) => {
    ...
    if (/(favorite|rating|rate|star)/i.test(text)) {
      return "You can save books with the heart button and rate a book from its detail page. If you need help with a specific rating issue, I can also transfer you to an agent.";
    }

    if (/(buy|purchase|cart|price|payment|order)/i.test(text)) {
      return "For purchases, open a book that is available for sale and use Add to Cart. For payment or order issues that need account-specific help, I can transfer you to an agent.";
    }

    if (/(hello|hi|hey|namaste)/i.test(text)) {
      return "Hi! I'm the BookWise AI assistant. I can help you discover books, explain ratings and favorites, or guide you through the site. What can I help you with?";
    }

    if (/(agent|human|person|support|problem|issue|complaint)/i.test(text)) {
      return "I can help with common BookWise questions first. If this needs account-specific or advanced assistance, use “Transfer to agent” below and your conversation will appear in the administrator support inbox.";
    }

    return "I can help with book discovery, recommendations, ratings, favorites, purchases, and general BookWise guidance. If you need advanced account-specific help, you can transfer this chat to an agent.";
  };

  const sendBotMessage = async (text) => {
    // Optional secure server-side AI endpoint. Never place an AI secret in Vite.
    const aiEndpoint = import.meta.env.VITE_AI_SUPPORT_ENDPOINT;
  ...
  const sendMessage = async () => {
    ...
    try {
      if (mode === "bot") {
        ...
        // Small delay keeps the interaction feeling conversational.
  ...
  const transferToAgent = async () => {
    if (transferring || mode === "agent") return;
    ...
    try {
      await createAgentConversation(
        "The customer chose Transfer to Agent from the BookWise AI assistant.",
      );
      setMessages((items) => [
        ...items,
        {
          ...
          sender_type: "bot",
          message:
            "You're now connected to the BookWise support queue. An administrator can continue this conversation. This chat will stay open until an administrator ends it.",
          created_at: new Date().toISOString(),
        },
      ]);
    } catch (err) {
      setError(err.message || "Unable to transfer to an agent.");
    } finally {
  ...
  return (
    <>
      <button
        type="button"
        className={`support-launcher ${open ? "open" : ""}`}
        onClick={() => setOpen(true)}
        aria-label="Open BookWise AI support"
      >
        <span className="support-launcher-icon">✦</span>
        <span>
          <strong>BookWise AI</strong>
          <small>Ask anything about BookWise</small>
        </span>
      </button>
        ...
        <div className="support-widget">
          ...
          <div className="support-status">
            <div>
              <strong>
                {user
                  ? `Signed in as ${user.name || "BookWise customer"}`
                  : "Guest support"}
              </strong>
              <span>
                {mode === "bot"
                  ? "Your questions stay in the assistant unless you request an agent."
          ...
          <div className="support-messages">
            {messages.length === 0 && mode === "bot" ? (
              <div className="support-ai-welcome">
                <div className="support-ai-welcome-icon">✦</div>
                <span className="support-ai-label">BOOKWISE AI</span>
                <h3>How can I help you today?</h3>
                <p>
                  Ask about books, recommendations, ratings, favorites, orders,
            ...
            {loading && (
              <div className="support-typing" aria-label="Loading">
                <span />
                <span />
                <span />
              </div>
          ...
          {mode === "agent" && (
            <div className="support-agent-banner">
              <span>●</span>
              <div>
          ...
          <div className="support-compose">
            ...
            <button
              ...
              onClick={sendMessage}
              disabled={sending || transferring || !draft.trim()}
          ...
          <div className="support-footer">
            {mode === "agent" ? (
              <button type="button" onClick={resetBotChat}>
                Start a new AI chat
              </button>
            ) : (
              <span>AI first · human support only when you request it</span>
            )}
            {conversationId && mode === "agent" && (
...
async function loadConversationForId(id, visitorToken, setMessages, setError) {
  try {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok)
      throw new Error(data.error || "Unable to load support chat.");
    setMessages(Array.isArray(data.messages) ? data.messages : []);
    setError("");
  } catch (err) {
    setError(err.message || "Unable to load support chat.");
...
function AdminDashboard({ user, token, onLogout, onBackToBookWise }) {
  ...
  const loadAdminData = async () => {
    try {
      ...
      if (results[4].status === "fulfilled") {
        ...
        setActivityError(
          results[4].reason?.message || "Unable to load recent activity.",
        );
      }
  ...
  const loadUsageAnalytics = async () => {
    try {
      ...
      setUsageError(err.message || "Unable to load usage analytics.");
    } finally {
      setUsageLoading(false);
    }
  };
  useEffect(() => {
    if (activeTab === "user-log") loadUsageAnalytics();
  }, [activeTab]);
  const formatDuration = (seconds) => {
    const total = Math.max(0, Number(seconds || 0));
    const m = Math.floor(total / 60);
    const s = Math.floor(total % 60);
    if (m >= 60) return `${Math.floor(m / 60)}h ${m % 60}m ${s}s`;
    return m ? `${m}m ${s}s` : `${s}s`;
  };
  const usageRows = useMemo(() => {
    const byUser = new Map();
    for (const row of usageAnalytics.users)
    ...
    for (const session of usageAnalytics.sessions) {
      const row = byUser.get(Number(session.user_id));
      if (!row) continue;
      const sec = Number(session.duration_seconds || 0);
      if (
        session.activity_type === "page" &&
        (session.resource_key === "dashboard" ||
          session.resource_name === "Dashboard")
      )
        row.dashboard_seconds += sec;
      else if (session.activity_type === "favorites")
        row.favorites_seconds += sec;
      else if (session.activity_type === "comments")
        row.comments_seconds += sec;
      else if (session.activity_type === "book") row.book_seconds += sec;
    }
    return Array.from(byUser.values()).filter(
      (row) => !usageUserFilter || String(row.id) === usageUserFilter,
    );
  ...
  const handleSaveBook = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...
        sale_price_npr:
          bookForm.sale_price_npr !== ""
            ? Number(bookForm.sale_price_npr)
            : null,
  ...
  const paymentSummary = useMemo(() => {
    ...
    for (const payment of payments) {
      ...
      if (
        ...
        ["failed", "canceled", "expired", "refunded"].includes(
  ...
  const openSupportConversation = async (conversation) => {
    ...
    try {
      const data = await adminFetch(
        `/api/admin/support/conversations/${conversation.id}`,
      );
  ...
  const replyToSupport = async () => {
    ...
    try {
      setSupportLoading(true);
      const data = await adminFetch(
        `/api/admin/support/conversations/${supportConversation.id}/messages`,
        {
          method: "POST",
  ...
  const closeSupportConversation = async () => {
    ...
    const confirmed = window.confirm(
      "End this conversation? The customer will no longer be able to continue this agent conversation.",
    );
    if (!confirmed) return;

    try {
      setClosingSupportId(supportConversation.id);
      setSupportError("");

      // The server owns conversation lifecycle; customers cannot close chats.
      await adminFetch(
        `/api/admin/support/conversations/${supportConversation.id}`,
        {
          method: "PATCH",
      ...
      setSupportError(
        err.message ||
          "Unable to end the conversation. Add the admin PATCH close route to the backend.",
      );
    } finally {
  ...
  const generateAdminInvite = async () => {
    try {
      ...
      const data = await adminFetch("/api/admin/invite-codes", {
        method: "POST",
      });
        ...
        <nav className="admin-nav">
          ...
          >
            ...
            {supportConversations.reduce(
              ...
              ? `(${supportConversations.reduce((sum, item) => sum + Number(item.unread_count || 0), 0)})`
              : ""}
          </button>
          ...
          <button
            type="button"
            className={
              activeTab === "team" ? "admin-nav-item active" : "admin-nav-item"
            }
      ...
      <main className="admin-main">
                  ...
                  <tbody>
                    {users.map((item, index) => (
                      <tr key={item.id || index}>
                        ...
                        <td>
                          {item.last_login
                            ? formatAdminDate(item.last_login)
                            : "Never"}
                        </td>
                        <td>
                          <button
                            type="button"
                            onClick={() => openUserActivity(item)}
                            style={{
                              padding: "7px 10px",
                              borderRadius: 7,
          ...
          <section className="admin-section">
            ...
            >
              {[
                [
                  ...
                  `NPR ${paymentSummary.pendingAmount.toLocaleString()}`,
                ],
                [
                  "Completed",
                  paymentSummary.completed,
                  `NPR ${paymentSummary.paidAmount.toLocaleString()}`,
                ],
            ...
            {paymentLoading ? (
              ...
              <div className="admin-empty">
                ...
                <p>
                  Transactions will appear here when customers start checking
                  out.
                </p>
              </div>
            ) : (
                    ...
                    {payments.map((payment) => {
                      ...
                      return (
                        <tr key={payment.id}>
                          ...
                          <td>
                            {formatAdminDateTime(payment.payment_created_at)}
                          </td>
                          <td>
                            {isPending &&
                            ["bank", "btc", "usdt"].includes(
        ...
        {selectedActivityUser && (
          <div
            ...
            onMouseDown={(e) => {
              if (e.target === e.currentTarget) setSelectedActivityUser(null);
            }}
          >
            <div
              style={{
            ...
            >
              ...
              >
                <div>
                  <span className="admin-eyebrow">USER AUDIT LOG</span>
              ...
              {userActivity && (
                <>
                  ...
                  >
                    {[
                      ["Logins", userActivity.summary?.login_count],
                      ["Comments", userActivity.summary?.comment_count],
                      ["Ratings", userActivity.summary?.rating_count],
                      ["Favorites", userActivity.summary?.favorite_count],
                      ["Total events", userActivity.summary?.total_events],
                    ].map(([label, value]) => (
                      <div
                        key={label}
                        style={{
                    ...
                    <table className="admin-table">
                      ...
                      <tbody>
                        {(userActivity.sessions || []).map((session) => (
                          <tr key={session.id}>
                            <td>{formatAdminDateTime(session.started_at)}</td>
                            ...
                            <td>
                              {formatAdminDateTime(session.last_heartbeat_at)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <table className="admin-table">
                      ...
                      <tbody>
                        {(userActivity.events || []).map((event) => (
                          <tr key={event.id}>
                            <td>{formatAdminDateTime(event.created_at)}</td>
                            <td>
                              <span className="activity-event-badge">
                                {String(event.event_type || "").replaceAll(
                                  "_",
                                  " ",
                                )}
                              </span>
          ...
          <section className="admin-section support-admin-section">
            <div className="admin-section-header">
              <div>
                ...
                <p>
                  Customers and guests can message you here. Administrators
                  cannot start customer conversations.
                </p>
              </div>
              <button
                type="button"
              ...
              <div className="support-conversation-list">
                {supportConversations.length === 0 ? (
                  <div className="admin-empty">
                    ...
                    <p>New customer messages will appear automatically.</p>
                  </div>
                ) : (
                  supportConversations.map((conversation) => (
                    <button
                      key={conversation.id}
                      type="button"
                      className={`support-conversation-item ${supportConversation?.id === conversation.id ? "active" : ""}`}
                    ...
                    >
                      ...
                      <small>
                        {conversation.customer_email || "Guest session"}
                        {conversation.user_id
                          ? ` · User #${conversation.user_id}`
                          : ""}
                      </small>
                      <div className="support-conversation-meta-row">
              ...
              <div className="support-admin-chat">
                {!supportConversation ? (
                  <div className="support-admin-empty">
                    ...
                    <p>Choose a customer from the left.</p>
                  </div>
                ) : (
                  <>
                    <div className="support-admin-chat-header">
                      <div className="support-admin-customer">
                        ...
                        <div>
                          <strong>
                            {supportConversation.customer_name ||
                              "Guest customer"}
                          </strong>
                          <span>
                            {supportConversation.customer_email || "Guest"}
                            {supportConversation.user_id
                              ? ` · User #${supportConversation.user_id}`
                              : ""}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="support-admin-header-actions">
                        ...
                        <button
                          ...
                          disabled={closingSupportId === supportConversation.id}
                        >
                          {closingSupportId === supportConversation.id
                            ? "Ending..."
                            : "End conversation"}
                        </button>
                      </div>
                    ...
                    <div className="support-admin-messages">
                      {supportLoading && supportMessages.length === 0 ? (
                        ...
                        supportMessages.map((item) => (
                          <div
                            key={item.id}
                            className={`support-admin-message ${item.sender_type}`}
                          >
                            <span>
                              {item.sender_type === "admin"
                                ? "You · Admin"
                    ...
                    <div className="support-admin-compose">
                      <input
                        value={supportDraft}
          ...
          <section className="admin-section">
            <div className="admin-section-header">
              <div>
                ...
                <p>
                  Only an existing administrator can generate the one-time
            ...
            <div className="admin-invite-card">
              <div>
                <span className="admin-eyebrow">NEW ADMIN INVITATION</span>
                <h3>Generate a 6-digit code</h3>
                <p>
                  The code is stored securely and expires automatically. Give it
                  only to the person you want to add.
                </p>
              </div>
              <button
                type="button"
              ...
              {inviteCode && (
                <div className="admin-invite-code">
                  ...
                  <small>
                    Expires in {inviteExpires}. It can be used once.
                  </small>
                  <button
                    type="button"
            ...
            <div className="admin-security-note">
              Never put the invitation code in frontend source code or .env
              files. It is generated by the authenticated admin session.
            </div>
          </section>
        )}
          ...
          <section className="admin-section admin-user-activity-page">
            <div className="admin-section-header">
              <div>
                ...
                <p>
                  Every tracked customer action appears here with the exact date
                  and time.
                </p>
              </div>
              <button
            ...
            <div className="activity-summary-strip">
              <div>
                <strong>{recentUserActivity.length}</strong>
                <span>Recent events</span>
              </div>
              <div>
                <strong>
                  {new Set(recentUserActivity.map((item) => item.user_id)).size}
                </strong>
              ...
              <div>
                <strong>
                  {
                    recentUserActivity.filter(
                      (item) => item.event_type === "login",
                    ).length
                  }
                </strong>
                <span>Logins</span>
              ...
              <div>
                <strong>
                  {
                    recentUserActivity.filter((item) =>
                      String(item.event_type || "").startsWith("comment_"),
                    ).length
                  }
                </strong>
                <span>Comments</span>
              ...
              <div>
                <strong>
                  {
                    recentUserActivity.filter((item) =>
                      String(item.event_type || "").startsWith("rating_"),
                    ).length
                  }
                </strong>
                <span>Ratings</span>
              ...
              <div className="admin-table-wrapper">
                {recentUserActivity.length === 0 ? (
                  <div className="admin-empty">
                    ...
                    <p>User log events will appear here automatically.</p>
                  </div>
                ) : (
                            ...
                            <td>
                              ...
                              >
                                <span className="admin-mini-avatar">
                                  {(item.user_name || item.user_email || "U")
                                    .charAt(0)
                                    .toUpperCase()}
                                </span>
                                <span>
                                  ...
                                  <small>{item.user_email || "Guest"}</small>
                                </span>
                              </button>
                            </td>
                            <td>
                              <span
                                className={`activity-event-badge activity-event-${String(item.event_type || "other").replace(/[^a-z0-9-]/gi, "-")}`}
                              >
                                {event}
                              </span>
                            </td>
        ...
        {activeTab === "user-log" && (
          <section className="admin-section">
            <div className="admin-section-header">
              <div>
                <span className="admin-eyebrow">USER ANALYTICS</span>
                <h2>Detailed User Log & Usage</h2>
                <p>
                  Real foreground time for Dashboard, Favorites, Comments and
                  Books.
                </p>
              </div>
              <button
            ...
            {usageError && (
              <div className="admin-error">
                <strong>Usage analytics error</strong>
            ...
            >
              ...
              <select
                ...
                onChange={(e) => setUsageActivityFilter(e.target.value)}
                style={{
                  padding: "10px 12px",
                  border: "1px solid #dbe2ea",
                  borderRadius: 10,
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {usageRows.length === 0 ? (
                    ...
                    usageRows.map((row) => {
                      const sessions = usageAnalytics.sessions.filter(
                        (s) =>
                          Number(s.user_id) === Number(row.id) &&
                          (usageActivityFilter === "all" ||
                            s.activity_type === usageActivityFilter),
                      );
                      const total = sessions.reduce(
                        (n, s) => n + Number(s.duration_seconds || 0),
                        0,
                      );
                      return (
                        <tr key={row.id}>
                          <td>
                            <button
                              type="button"
                              className="activity-user-cell"
                              onClick={() => openUserActivity(row)}
                            >
                              <span className="admin-mini-avatar">
                                {(row.name || row.email || "U")
                                  .charAt(0)
                                  .toUpperCase()}
                              </span>
                              <span>
                          ...
                          <td>
                            <strong>
                              {formatAdminDateTime(row.last_login)}
                            </strong>
                            <small
                              style={{ display: "block", color: "#64748b" }}
                            >
                              {row.login_count || 0} login(s)
                            </small>
                          </td>
                          <td>{formatAdminDateTime(row.last_seen)}</td>
                          <td>
                            <strong>{formatDuration(total)}</strong>
                          </td>
                          <td>{formatDuration(row.dashboard_seconds)}</td>
                          <td>{formatDuration(row.favorites_seconds)}</td>
                          <td>{formatDuration(row.comments_seconds)}</td>
                          <td>{formatDuration(row.book_seconds)}</td>
            ...
            >
              <h3 style={{ marginTop: 0 }}>Book & Page Time Ranking</h3>
              <div className="admin-table-wrapper">
                <table className="admin-table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {[
                      ...usageAnalytics.books.map((x) => ({
                        ...x,
                        type: "Book",
                        name: x.book,
                      })),
                      ...usageAnalytics.pages.map((x) => ({
                        ...x,
                        type: "Page",
                        name: x.page,
                      })),
                      ...
                      .slice(0, 100)
                      .map((x, i) => (
                        <tr key={`${x.type}-${x.name}-${i}`}>
                          <td>{x.type}</td>
                          <td>
                            <strong>{x.name}</strong>
                          </td>
                          <td>{x.visits}</td>
                          <td>{formatDuration(x.total_seconds)}</td>
                          <td>{formatDuration(x.avg_seconds)}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
...
function App() {
  ...
  // Hero books are selected only after the API successfully returns books.
  // The initial fallback keeps the hero visible while the database loads.
  ...
  useUsageTracker({
    token,
    enabled: Boolean(
      token && !adminPage && !adminAuthPage && !bookPage && !favoritesOpen,
    ),
    activityType: "page",
  ...
  const loadBooks = async () => {
    try {
      ...
      // Pick 3 different database books with covers on every successful
      ...
      const selectedHeroBooks = shuffled.slice(0, 4);

      if (selectedHeroBooks.length === 4) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setHeroOffset((current) => (current + 1) % heroBooks.length);
    }, 5000);
   ...
   * shelves still use books returned by /api/books rather than hard-coded data.
   */
  const databaseSuggestedBooks = useMemo(() => {
    const favoriteIds = favorites || new Set();

    return [...books]
      .filter((book) => {
        const id = getBookIdentifier(book);
        return id ? !favoriteIds.has(String(id)) : true;
      })
      .sort((a, b) => {
  ...
  // loaded by /api/books. This prevents recommendation/discovery endpoints
  // from showing a different book collection from the main database shelf.
  const displaySuggestedBooks = databaseSuggestedBooks;

  const displayPopularBooks = databasePopularBooks;
  ...
  const loadOrderStatus = async (orderId) => {
    ...
    const data = await response.json();
    if (!response.ok)
      throw new Error(data.error || "Could not verify order status");
    return data.order;
  };
    ...
    try {
      ...
      if (["bank", "btc", "usdt"].includes(provider)) {
        const paymentResponse = await fetch(
          `${API_URL}/api/payments/manual/submit`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${currentToken}`,
        ...
        if (!paymentResponse.ok)
          throw new Error(paymentData.error || "Could not submit payment");
        ...
        setPaymentNotice({
          ...
          provider,
          orderId,
          message: `Payment recorded for Order #${paymentData.order.order_number}. It is pending manual verification.`,
        });
        return;
      }

      const paymentResponse = await fetch(
        `${API_URL}/api/payments/${provider}/initiate`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${currentToken}`,
  ...
  useEffect(() => {
    ...
    if (!payment || !provider || !Number.isInteger(orderId) || orderId <= 0)
      return;
      ...
      try {
        ...
        if (order.status === "paid" || payment === "success") {
          setPaymentNotice({
            ...
            provider,
            orderId: order.id,
            message: `Payment confirmed successfully. Order #${order.order_number} is paid.`,
          });
          ...
          setPaymentNotice({
            ...
            provider,
            orderId: order.id,
            message: `Payment is still pending. We have not marked Order #${order.order_number} as paid.`,
          });
        } else {
          setPaymentNotice({
            type: "error",
            provider,
            orderId: order.id,
            message: `Payment was not completed. Order #${order.order_number} remains unpaid.`,
          });
        }
      } catch (error) {
        ...
        if (!cancelled) {
          setPaymentNotice({
            ...
            orderId,
            message:
              "We received the payment return, but could not confirm the final status yet. Please check again before retrying.",
          });
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <>
          ...
          <nav className="navbar">
            <div className="nav-container">
              ...
              <div className="nav-actions">
                ...
                <button
                  type="button"
                  className="nav-btn admin-portal-nav-btn"
                  onClick={goToAdmin}
                >
                  🛡️{" "}
          ...
          {!search && (
            <header className="hero">
              ...
              <div className="hero-books" aria-hidden="true">
                <div className="hero-book-glow hero-book-glow-one" />
      ...
      {!adminPage && !adminAuthPage && (!user || user.role !== "admin") && (
        <SupportWidget user={user} books={books} />
      )}
